...
$PATH = __DIR__."/admin/";
require_once __DIR__.'/public_html/admin/assets/mysqlconnector.php';
$ctype = '';
$p = 1;
if(isset($_GET['ctype'])) {
    $ctype=$_GET['ctype']; 
...
<div id="top-container" class="stretch">
    ...
    <div id="maindiv" data-open="">
        ...
        <div id="partners" class="showdiv">
            <div class="innerdiv">
                <?php
                 $stmt=$c['db']->query("SELECT * FROM pages where name='partners' and parentid=1");
                 $res=$stmt->fetch(PDO::FETCH_ASSOC);
                 echo $res['description'];
                ?>
            </div>
    ...
    <div class="container partners">
        <div id="slides">
        <?php
        $stmt=$c['db']->query("SELECT count(*) FROM sponsors ORDER BY rank");
        $res=$stmt->fetch();
        $content="";
        
        if($res[0]!=0)
        {
            $stmt=$c['db']->query("SELECT * from sponsors ORDER BY rank");
            while($res=$stmt->fetch())
            {
                $content .= "<a href='{$res['url']}' target='_blank'><img src='./sponsors/{$res['image']}'></a>";
            }
        }
        echo $content;
        ?>
